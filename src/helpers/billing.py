...
if "sk_test" in STRIPE_SECRET_KEY and not DJANGO_DEBUG:
    raise ValueError("You are using the Stripe test secret key. Please use the live secret key.")
...
def start_checkout_session(
    ...
    if not success_url.endswith("={CHECKOUT_SESSION_ID}"):
        success_url=f"{success_url}"+ "={CHECKOUT_SESSION_ID}"
...
def cancel_subscription(stripe_id, reason="", feedback="", cancel_at_period_end=False, raw = True):
